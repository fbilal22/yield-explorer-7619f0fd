...
function parseFuturesFromMarkdown(markdown: string): FuturesData[] {
  ...
  // Log first 50 lines to debug parsing
  console.log('First 100 lines of markdown:');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Updated pattern to handle various contract formats like J8Z25, T0Z25, SQZ25, RAZ25
    const contractMatch = line.match(/\[([A-Z0-9]{2,4}\w*)\s*\(([^)]+)\)\]/);
    ...
    if (contractMatch) {
      ...
      dataBuffer = [];
      inTable = true;
      console.log('Found contract:', currentContract.contract);
